...
def get_chat_client() -> OpenAIChatClient:
    """Return Azure AI Foundry chat client using the OpenAI v1 API with managed identity."""
    ...
    for suffix in ('/responses', '/chat/completions', '/openai/v1'):
    ...
    # The /openai/v1 path rejects the api-version query string that the Azure
    # OpenAI client always appends, so use a plain AsyncOpenAI client and inject
    # the bearer token via an httpx request hook (so it refreshes per call).
    async def _add_auth(request: httpx.Request) -> None:
        request.headers["Authorization"] = f"Bearer {token_provider()}"

    http_client = httpx.AsyncClient(event_hooks={"request": [_add_auth]})
